...
class OAuth2Provider : AuthenticationProvider {
    ...
    private val oAuth2ClientMap: MutableMap<String, OAuth2Client> = HashMap()
    ...
    override fun authenticate(authentication: Authentication): Authentication {
        ...
        if (response!!.status != HttpStatus.OK) throw UsernameNotFoundException("User token failed to authenticate on " + request.authProvider)
    ...
    private fun loadAndUpdate(response: OAuth2Response?): User? {
        val user = authUserService!!.loadUserByEmail(response!!.email)
                .orElseThrow { UsernameNotFoundException(String.format("Authentication success, but user email '%s' is not found in database", response.email)) }
